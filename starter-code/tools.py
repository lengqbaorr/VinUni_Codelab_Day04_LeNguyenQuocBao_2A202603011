...
def submit_support_ticket(
    ...
    # Load existing tickets (tránh Trap 2: không được ghi đè)
    existing_tickets = []
...
TOOL_DEFINITIONS = [
    {
        ...
        "description": "Tra cứu sản phẩm/dịch vụ Vingroup theo danh mục và giá tối đa. Gọi tool này khi khách hỏi về xe điện VinFast hoặc gói du lịch Vinpearl.",
        "parameters": {
            "type": "object",
            "properties": {
                "category": {
                ...
                "max_price": {
                    "type": "integer",
                    "description": "Giá tối đa tính bằng VNĐ. Bỏ qua nếu không có giới hạn giá."
                }
            },
            "required": ["category"]
        }
    },
    {
        ...
        "description": "Tạo ticket hỗ trợ khách hàng và lưu vào hệ thống. Gọi tool này khi khách yêu cầu được hỗ trợ, báo lỗi, hoặc khiếu nại.",
        "parameters": {
            "type": "object",
            "properties": {
                "customer_name": {
                ...
                "issue_description": {
                    "type": "string",
                    "description": "Mô tả chi tiết vấn đề hoặc yêu cầu hỗ trợ của khách hàng."
                },
                "priority": {
                    "type": "string",
